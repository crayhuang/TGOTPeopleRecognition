...
for i in range(0, 10):
    for rt, dirs, files in os.walk(images_path):
...
with tf.Session() as sess:
    ...
    print("一共读取了%s个输入图像，%s标签" % (input_count, input_count))
    batch_size = 60
    iterations = 100
    batches_count = int(input_count / batch_size)
    remainder = input_count % batch_size
    print("数据分%s批, 前面每批%s个数据，最后一批%s个数据" % (batches_count + 1, batch_size, remainder))
